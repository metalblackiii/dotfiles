...
Never start work without checking the skills list. Matching skill → invoke it first, work second.
...
- **Prefer built-in tools over Bash equivalents** — `Read` not `cat`, `Glob` not `fd`/`ls`/`find`, `Grep` not `rg`/`grep`.
...
- Default reviewers: `phoenix` team in the repo's org (`Chiropractic-CT-Cloud/phoenix` or `PracticeTek-Common-Services/phoenix`)
...
- GitHub usernames follow the format `firstname-lastname_ptek`
