...
# Format: action_name=current_version:minimum_version (one per line)
# Portable: no associative arrays (Bash 3.2 compat for macOS)
ACTION_VERSION_DB="
...
# Lookup version info for an action. Returns "" if not found.
lookup_action_version() {
    local action="$1"
    echo "$ACTION_VERSION_DB" | grep -F "${action}=" | head -1 | sed "s/^.*=//"
}
    ...
    for file in "${files_to_check[@]}"; do
        ...
        while IFS= read -r line; do
            ...
            if [[ "$line" =~ uses:[[:space:]]*([^@]+)@([^[:space:]#]+) ]]; then
                ...
                local version_info
                version_info="$(lookup_action_version "$action")"
...
test_with_act() {
    ...
    if [ -f "$abs_workflow_path" ]; then
        ...
        if [[ "$abs_workflow_path" == *"/.github/workflows/"* ]]; then
            # File is in .github/workflows - store relative path for -W flag
            workflow_flag="${abs_workflow_path#$repo_root/}"
    ...
    if [ -n "$workflow_flag" ]; then
        list_cmd+=("-W" "$workflow_flag")
    fi
    list_cmd+=("${runner_images[@]}")
    log_info "Running: act --list${workflow_flag:+ -W $workflow_flag}"
    if ! "${list_cmd[@]}" 2>&1 | head -30; then
    ...
    local -a dryrun_cmd=("${act_path}" "--dryrun")
    if [ -n "$workflow_flag" ]; then
        dryrun_cmd+=("-W" "$workflow_flag")
    fi
    dryrun_cmd+=("--container-architecture" "linux/amd64" "${runner_images[@]}")
    log_info "Running: act --dryrun${workflow_flag:+ -W $workflow_flag} --container-architecture linux/amd64"
    act_output=$("${dryrun_cmd[@]}" 2>&1)
    act_exit_code=$?
