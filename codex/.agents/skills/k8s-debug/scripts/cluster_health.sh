...
run_pipe_or_warn "Cluster version" bash -o pipefail -c 'kubectl --request-timeout="$1" version --client=false 2>/dev/null || kubectl --request-timeout="$1" version' _ "$REQUEST_TIMEOUT"
...
run_pipe_or_warn "Recent events query" bash -o pipefail -c 'kubectl --request-timeout="$1" get events --all-namespaces --sort-by=.lastTimestamp | tail -50' _ "$REQUEST_TIMEOUT"
...
run_pipe_or_warn "Component readiness endpoint query" bash -o pipefail -c 'kubectl --request-timeout="$1" get --raw=/readyz?verbose 2>/dev/null || kubectl --request-timeout="$1" get --raw=/healthz?verbose 2>/dev/null || kubectl --request-timeout="$1" get componentstatuses' _ "$REQUEST_TIMEOUT"
