...
api_probe_secure() {
    ...
    # Pass token via env to avoid leaking it in process args visible to ps/proc
    if pod_exec env "AUTH_HEADER=Authorization: Bearer $token" \
        sh -c 'curl --fail --silent --show-error --cacert "$1" --max-time 5 -H "$AUTH_HEADER" "$2"' \
        _ "$SERVICEACCOUNT_CA" "$KUBERNETES_API_URL" >/dev/null 2>&1; then
        return 0
    fi

    if pod_exec env "AUTH_HEADER=Authorization: Bearer $token" \
        sh -c 'wget -q --timeout=5 --ca-certificate="$1" --header="$AUTH_HEADER" -O /dev/null "$2"' \
        _ "$SERVICEACCOUNT_CA" "$KUBERNETES_API_URL" >/dev/null 2>&1; then
        return 0
    fi
...
api_probe_insecure() {
    ...
    if [ -n "$token" ]; then
        # Pass token via env to avoid leaking it in process args visible to ps/proc
        if pod_exec env "AUTH_HEADER=Authorization: Bearer $token" \
            sh -c 'curl --fail --silent --show-error -k --max-time 5 -H "$AUTH_HEADER" "$1"' \
            _ "$KUBERNETES_API_URL" >/dev/null 2>&1; then
            return 0
        fi
        if pod_exec env "AUTH_HEADER=Authorization: Bearer $token" \
            sh -c 'wget -q --timeout=5 --no-check-certificate --header="$AUTH_HEADER" -O /dev/null "$1"' \
            _ "$KUBERNETES_API_URL" >/dev/null 2>&1; then
            return 0
        fi
    else
...
run_pipe_or_warn "Pod describe network details query" bash -o pipefail -c 'kubectl --request-timeout="$1" describe pod "$2" -n "$3" | grep -A 20 "^IP:"' _ "$REQUEST_TIMEOUT" "$POD_NAME" "$NAMESPACE"
