...
install_tools() {
    ...
    TEMP_INSTALL=true
    VENV_BASE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/dockerfile-expert.XXXXXX")
...
run_best_practices() {
    ...
    if grep -qiE "^[[:space:]]*FROM[[:space:]]+[^[:space:]]+:latest([[:space:]]|$)" "$DOCKERFILE"; then
        ...
        BP_WARNINGS=$((BP_WARNINGS + 1))
    fi

    # Check for USER directive in the final build stage only.
    # In multi-stage builds, USER in a builder stage doesn't affect the runtime stage.
    local final_stage_content
    final_stage_content=$(printf '%s\n' "$normalized_content" | awk '
        BEGIN { IGNORECASE=1 }
        /^[[:space:]]*FROM[[:space:]]+/ { buf = "" }
        { buf = buf $0 "\n" }
        END { printf "%s", buf }
    ')

    if ! printf '%s\n' "$final_stage_content" | grep -qiE "^[[:space:]]*USER[[:space:]]+"; then
        if is_nonroot_base_image "$final_image"; then
            echo -e "${PURPLE}[INFO] No USER directive in final stage, but base image is non-root: $final_image${NC}"
            echo "  → Confirm runtime user requirements for your platform"
            BP_INFO=$((BP_INFO + 1))
        else
            echo -e "${YELLOW}[WARNING] No USER directive in final stage - container will run as root${NC}"
            echo "  → Add 'USER <non-root-user>' before CMD/ENTRYPOINT in the final stage"
            BP_WARNINGS=$((BP_WARNINGS + 1))
        fi
    else
        LAST_USER=$(printf '%s\n' "$final_stage_content" | grep -iE "^[[:space:]]*USER[[:space:]]+" | tail -n1 | awk '{print $2}')
        LAST_USER_LOWER=$(echo "$LAST_USER" | tr '[:upper:]' '[:lower:]')
        if [[ "$LAST_USER_LOWER" == "root" ]] || [[ "$LAST_USER" == "0" ]] || [[ "$LAST_USER" == "0:0" ]]; then
            echo -e "${RED}[ERROR] Last USER directive in final stage sets user to root${NC}"
            echo "  → Container should not run as root user"
            BP_ERRORS=$((BP_ERRORS + 1))
            EXIT_CODE=1
        fi
    fi
