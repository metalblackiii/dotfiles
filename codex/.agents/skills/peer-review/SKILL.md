---
name: peer-review
description: Use when you want a peer review of local git changes without implementation context bias.
...
`peer-review` is orchestration only. It enforces context isolation, then delegates review logic to existing skills. Review standards stay anchored to `self-review` and `analyzing-prs`.
...
- `../analyzing-prs/SKILL.md` (review categories and severity taxonomy)
...
4. Build a bundle containing only:
   ...
   - `analyzing-prs` criteria + severity definitions
...
- **Claude Code:** dispatch with Task/fork using the same bundle-only payload.
...
3. Apply `self-review` workflow and `analyzing-prs` criteria.
