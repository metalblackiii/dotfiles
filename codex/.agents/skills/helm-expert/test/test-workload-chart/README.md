...
A test Helm chart with **intentional Stage 9 security violations** for use with the `helm-expert` skill.

## Purpose

This chart exists to exercise and demonstrate the **mandatory Stage 9 Security Best Practices Check** in the `helm-expert` skill workflow. It renders a valid `Deployment` that deliberately omits every security control the skill is expected to flag.
...
```bash
# From the helm-expert skill directory:
