...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    if name == "get_weather":
        ...
        return [
            TextContent(
                type="text",
                text=f"Weather in {args.location}: {weather_data['temp']}°{'C' if args.units == 'celsius' else 'F'}",
            )
        ]
...
@app.call_tool()
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    logger.info(f"Tool called: {name}")  # Never log arguments — may contain secrets or PHI
