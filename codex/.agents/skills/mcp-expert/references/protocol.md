...
```typescript
// Client POSTs JSON-RPC requests to the MCP endpoint
// Server responds inline or upgrades to SSE for streaming
// Used for remote servers — replaces the legacy HTTP+SSE transport
...
// Server may respond with JSON directly (single response)
// or with Content-Type: text/event-stream for streaming
```

> **Legacy note:** Older docs reference separate `POST /mcp` + `GET /mcp/sse` endpoints (HTTP+SSE transport). This was replaced by Streamable HTTP. The SDKs handle this automatically — use the latest SDK version.

## Protocol Versions

Check the [MCP specification](https://modelcontextprotocol.io/specification) for the current version. Servers must declare their supported version in the initialize response. Clients should verify compatibility.
