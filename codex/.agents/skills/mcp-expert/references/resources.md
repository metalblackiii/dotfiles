...
ALLOWED_TABLES = {"users", "orders", "products"}  # allowlist — never interpolate arbitrary input
...
async def read_resource(uri: str) -> str:
    if uri.startswith("db://"):
        ...
        if resource_type == "data":
            rows = await db.query_table(table, limit=100)  # use parameterized query, not f-string
