#!/usr/bin/env bash
#
# Deterministic CI runner for bash-expert (validator).
...
if ! command -v shellcheck >/dev/null 2>&1; then
    ...
    echo "        Install shellcheck and re-run scripts/run_validator_ci.sh." >&2
    exit 1
fi

echo "[INFO] Running deterministic bash-expert validator regression suite..."
CI=1 \
