...
Architecture overview (layers, environments, services, shared libraries) is inlined below. Service conventions (controller patterns, directory layout, database access, cross-service communication, auth, testing) are provided by the `neb-ms-conventions` skill loaded into this agent.
...
1. **Discover available repos** using the base path above (`~/repos/`)
